define([
    ...
    class DataBase {
        ...
        executeQuery(sql, exitingTransaction ) {
           // console.log(`executeQuery() for table ${sql} existing transaction?`,exitingTransaction)
            return new Promise((resolve, reject) => {
                exitingTransaction ? exitingTransaction.executeSql(sql, [],
                    ...
                    tx.executeSql(sql, [],
                        ...
                        (error) => reject(`Error executing a statment ${error}`))
                }, (error) => reject(`Error opening a transaction ${error}`))
            })
        }

        create({ tableName, cols }, exitingTransaction) {
           // console.log(`create() for table ${tableName} existing transaction?`,exitingTransaction)
            let columns = ""
        ...
        getStatement({ operationType = 'select', tableName, tableColumns, condition = null, tableData }) {
            ...
            switch (operationType.toUpperCase()) {
                case SQL_OPERATION['SELECT']:
                    return condition ? `SELECT ${tableColumns} FROM ${tableName} WHERE ${condition}` : `SELECT * FROM ${tableName}`
        ...
        insert({ tableName, tableColumns, tableData }, exitingTransaction) {
            return new Promise((resolve, reject) => {
                ...
                this.executeQuery(stmt, exitingTransaction).then((result) =>  resolve(result), (err) => reject(err))
            })
        }
